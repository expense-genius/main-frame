...
export default function DashboardPage() {
	...
	if (!user && !isLoggingOut) {
		return <RedirectHandler redirectTo="/sign-in" redirectPageName="the sign-in page" />;
	}
		...
		<div className="min-h-screen bg-gray-100 text-gray-900">
			<div className="container mx-auto p-6">
				<div className="flex justify-between items-center">
					...
					<BasicButton name="Log Out" onClick={handleLogout} theme="danger" />
				</div>
							...
							<tbody>
								{transactions.map((transaction) => (
									<tr key={transaction.transaction_id} className="hover:bg-gray-100">
										...
										<td className="p-3 border">${transaction.amount.toFixed(2)}</td>
										<td className="p-3 border">{transaction.merchant_name || transaction.name}</td>
										<td className="p-3 border">{transaction.payment_channel}</td>
									</tr>
								))}
							</tbody>
						</table>
